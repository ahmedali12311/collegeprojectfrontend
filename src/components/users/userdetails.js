  ...
  const handleRoleToggle = (role) => {
    setFormData((prev) => {
      ...
      if (currentRoles.includes(roleValue)) {
        // Unselect the role
        currentRoles.splice(currentRoles.indexOf(roleValue), 1);
      } else {
        // Select the role
      ...
      if (
        (roleValue === 'admin' || roleValue === 'teacher') &&
        (currentRoles.includes('admin') || currentRoles.includes('teacher'))
      ) {
        // Remove student
        return {
          ...prev,
              ...
              {isEditing ? (
                <div className="profile-avatar-container">
                  <label htmlFor="image-upload">
                    <img 
                      src={formData.imageUrl || user?.image || 'https://via.placeholder.com/150'} 
